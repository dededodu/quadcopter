...
const unsigned long throttle_lost_sig_pulse_lim_l = ((throttle_lost_sig_pulse * 99) / 100); // 1% margin
const unsigned long throttle_lost_sig_pulse_lim_h = ((throttle_lost_sig_pulse * 101) / 100); // 1% margin
const unsigned long axis_lost_sig_pulse = 1500;
const unsigned long axis_lost_sig_pulse_lim_l = ((axis_lost_sig_pulse * 99) / 100); // 1% margin
const unsigned long axis_lost_sig_pulse_lim_h = ((axis_lost_sig_pulse * 101) / 100); // 1% margin
...
const uint8_t portd_mask = (uint8_t)(1 << MOTOR2_PIN) | (uint8_t)(1 << MOTOR1_PIN);
const uint8_t portb_mask = (uint8_t)(1 << (MOTOR4_PIN - 8)) | (uint8_t)(1 << (MOTOR3_PIN - 8));
...
const unsigned long rc_signal_emergency_trigger = rc_signal_timeout_us / loop_period;
...
bool rcSignalEmergency() {
    ...
    if (rc_no_signal_cnt[THROTTLE] > rc_signal_emergency_trigger ||
        ...
        if (rc_throttle <= MOTOR_MIN_PULSE) {
            ...
            emergency_throttle_step_us = emergency_landing_time_us / (rc_throttle - MOTOR_MIN_PULSE);
        }
 ...
 * Yaw rate is positive when motor 2 moves towards 1 (2 -> 1, 3 -> 2, 4 -> 3, 1 -> 4) 
 *
 */
void computeOutputs() {
#ifdef DEBUG
    ...
    if (rc_throttle < throttle_threshold) {
        ...
        previous_rate_error[PITCH] = previous_rate_error[ROLL] = previous_rate_error[YAW] = 0;
        motor_pulse[MOTOR1] = motor_pulse[MOTOR2] = motor_pulse[MOTOR3] = motor_pulse[MOTOR4] = rc_throttle;
    } else {
        pitch_pid = minMax((angle_error[PITCH] * Ki[PITCH]) + (rate_error[PITCH] * Kp[PITCH]) + (acc_error[PITCH] * Kd[PITCH]), -MAX_PULSE_CHANGE, MAX_PULSE_CHANGE);
        roll_pid = minMax((angle_error[ROLL] * Ki[ROLL]) + (rate_error[ROLL] * Kp[ROLL]) + (acc_error[ROLL] * Kd[ROLL]), -MAX_PULSE_CHANGE, MAX_PULSE_CHANGE);
        yaw_pid = minMax((angle_error[YAW] * Ki[YAW]) + (rate_error[YAW] * Kp[YAW]) + (acc_error[YAW] * Kd[YAW]), -MAX_PULSE_CHANGE, MAX_PULSE_CHANGE);

        motor_pulse[MOTOR1] = minMax(rc_throttle - roll_pid - pitch_pid + yaw_pid, MOTOR_MIN_RUNNING_PULSE, MOTOR_MAX_PULSE);
        motor_pulse[MOTOR2] = minMax(rc_throttle + roll_pid - pitch_pid - yaw_pid, MOTOR_MIN_RUNNING_PULSE, MOTOR_MAX_PULSE);
        motor_pulse[MOTOR3] = minMax(rc_throttle + roll_pid + pitch_pid + yaw_pid, MOTOR_MIN_RUNNING_PULSE, MOTOR_MAX_PULSE);
        motor_pulse[MOTOR4] = minMax(rc_throttle - roll_pid + pitch_pid - yaw_pid, MOTOR_MIN_RUNNING_PULSE, MOTOR_MAX_PULSE);
    }
...
void initMotors() {
    ...
    // Initialize ESCs
    motor_pulse[MOTOR1] = motor_pulse[MOTOR2] = motor_pulse[MOTOR3] = motor_pulse[MOTOR4] = MOTOR_MIN_PULSE;
